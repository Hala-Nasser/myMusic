...
import kotlinx.android.synthetic.main.fragment_national_song.*
import kotlinx.android.synthetic.main.fragment_national_song.view.*

/**
 * A simple [Fragment] subclass.
 */
class NationalSongFragment : Fragment(), SongAdapter.onSongItemClickListener {

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val root=inflater.inflate(R.layout.fragment_national_song, container, false)


        val db= DatabaseHelper(activity!!)
        val data=db.getNationalSongs()
        val songAdapter = SongAdapter(activity!!,data,this)
        root.rvNationalSong.layoutManager = GridLayoutManager(activity!!,2)
        root.rvNationalSong.setHasFixedSize(true)
        root.rvNationalSong.adapter=songAdapter
