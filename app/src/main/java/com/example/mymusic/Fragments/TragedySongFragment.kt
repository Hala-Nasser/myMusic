...
import kotlinx.android.synthetic.main.fragment_tragedy_song.view.*

/**
 * A simple [Fragment] subclass.
 */
class TragedySongFragment : Fragment(), SongAdapter.onSongItemClickListener {

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val root=inflater.inflate(R.layout.fragment_tragedy_song, container, false)

        val db= DatabaseHelper(activity!!)
        val data=db.getTragedySongs()
        val songAdapter = SongAdapter(activity!!,data,this)
        root.rvTragedySong.layoutManager = GridLayoutManager(activity!!,2)
        root.rvTragedySong.setHasFixedSize(true)
        root.rvTragedySong.adapter=songAdapter
